...
export default async function Page({ params }: PageProps<"/admin/timeline/[id]">) {
  const { id } = await params;
  const config = getCollection("timeline");
  const initial = id === "new" ? null : await getRow("timeline", id);
