...
export function MmsArchiveCards({locale, categories}: {locale:string;categories:string[]}) {
 return <section className="my-10">
  ...
  <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">{mmsRecords.filter(r=>categories.includes(r.category)).map(r=><Link key={r.id} href={`/${locale}/mms-hub/${r.id}`} className="block border-b border-line pb-6">
